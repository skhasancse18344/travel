  ...
  function onSelect() {
    ...
    if (isSelected) {
      ...
      if (destinations.length >= trip.totalDestination) {
        setAlertMessage(
          <span>
           Destination exceeds quota. You can delete other Destinations first
          </span>
        );
        setIsAlertShowed(true);
        return;
            ...
            <div className="flex flex-col">
              ...
              {(data.title == "Dagi Abhinaya by Manohara" ||
                ...
                <div className="flex flex-row items-start text-xs text-orangeSoft mt-1">
                  <div className="text-sm leading-tight mr-0.5">*</div>
                  <p>Reservations are required for this destination</p>
                </div>
              )}
...
function AdditionalInformation({ data }: { data: DestinationInterface }) {
  ...
  if (isEducation && hasAdditionalPrice) {
    return (
      <p>
        There is an additional fee of around Rp{" "}
    ...
    return (
      <p>
        There is an additional fee for the entrance ticket of around Rp{" "}
        <span className="font-semibold">{KFormatter(data.price!)}</span>/people
      </p>
    );
  } else if (isRestaurant) {
    return <p>The tour fee does not include food and drinks at this place</p>;
  } else {
    return <p>There are no additional fees for this destination</p>;
  }
}
